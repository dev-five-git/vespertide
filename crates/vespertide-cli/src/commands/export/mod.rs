...
use vespertide_exporter::{
    Orm, drizzle, gorm::GormExporterWithConfig, prisma, python_naming::to_pascal_case,
...
pub async fn cmd_export(orm: Orm, export_dir: Option<PathBuf>) -> Result<()> {
    ...
    // Prisma, Drizzle and GORM use a single-file output strategy
    ...
    if matches!(orm, Orm::Gorm) {
        return cmd_export_models_file(normalized_models, target_root).await;
    }
...
/// GORM gets the whole schema as one `models.go`. Go reads a directory as one
/// package and a relation is rendered from both of its ends, so models spread
/// over directories would import each other in a cycle.
///
/// A fixed file name also leaves nothing to sweep, so the user's own `.go`
/// files are never touched. The file itself is only overwritten when it
/// starts with [`GENERATED_MARKER`].
async fn cmd_export_models_file(
    normalized_models: Vec<(TableDef, PathBuf)>,
    target_root: PathBuf,
) -> Result<()> {
    let all_tables: Vec<TableDef> = normalized_models.iter().map(|(t, _)| t.clone()).collect();
    let code = GormExporterWithConfig::for_export_dir(&target_root)
        .export(&all_tables)
        .map_err(|e| anyhow::anyhow!(e))?;
    let marker = format!("// {GENERATED_MARKER}");

    let out_path = target_root.join("models.go");
